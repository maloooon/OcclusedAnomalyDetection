...
# The two dataclasses and get_boxes function are only for grounding SAM, I implemented that based on a google colab by the authors, but I do not find the link anymore, sorry
@dataclass
class BoundingBox:
    xmin: int
    ymin: int
...
def model_SAM3(samples, save_imgs_bool=False, store_masks_bool=False, testing_samples=1, filter_bboxes = (True, 0.2, 3.0), filter_masks_shapes = (True, 0.85), filter_masks_sizes = (True, 0.2, None), prompt = "raspberry", filter_red=(True, 0.4), filter_holes_islands=False, filter_overlap_masks=False):
    ...
    time_model_load = time() - _t_load_start
    print(f"SAM3 model loading time (one-off): {time_model_load:.2f} seconds")
    ...
    if filter_overlap_masks:
        ...
        time_depth_load = time() - _t0
        print(f"Depth pipeline loading time (one-off): {time_depth_load:.2f} seconds")
        print(f"Depth-Anything parameter dtypes: { {p.dtype for p in depth_pipe.model.parameters()} }")
    ...
    for loop_idx, sample in enumerate(samples):
        ...
        if loop_idx == 0:
            print(f"SAM3 parameter dtypes: { {p.dtype for p in predictor.model.parameters()} }")
        if loop_idx > 0:
...
#        )
 ...
 #       )
    ...
    if times_image_encoding:
        ...
        print(f"Model load time (one-off): {time_model_load:.3f} s")
        if filter_overlap_masks:
            print(f"Depth load time (one-off): {time_depth_load:.3f} s")
...
def model_SAM_extended(samples, mode='mobile', save_imgs_bool=False, store_masks_bool=False, testing_samples=1, filter_bboxes=(True, 0.2, 3.0), filter_masks_shapes=(True, 0.85), filter_masks_sizes=(True, 0.2, None), filter_red=(True, 0.4), filter_holes_islands=False, filter_overlap_masks=False):
    ...
    time_model_load = time() - _t_load_start
    print(f"SAM model loading time (one-off): {time_model_load:.2f} seconds")
    ...
    time_punnet_load = time() - _t0
    print(f"YOLO punnet model loading time (one-off): {time_punnet_load:.2f} seconds")
    print(f"YOLO punnet parameter dtypes: { {p.dtype for p in punnet_model.model.parameters()} }")
    ...
    for loop_idx, sample in enumerate(samples):
       ...
       # exit()
        ...
        print(f"Image encoding time for sample {sample_idx}: {end_time_0 - start_time_0:.2f} seconds")
        if loop_idx == 0:
            print(f"SAM parameter dtypes: { {p.dtype for p in predictor.model.parameters()} }")
        if loop_idx > 0:
            times_image_encoding.append(end_time_0 - start_time_0)
        start_time = time()
    ...
    if times_image_encoding:
        ...
        if filter_overlap_masks:
            print(f"Depth load time (one-off): {time_depth_load:.3f} s")
        print(f"YOLO load time (one-off) : {time_punnet_load:.3f} s")
        print(f"Avg image encoding time  : {avg_enc:.3f} s")
        print(f"Avg inference time       : {avg_inf:.3f} s")
        if times_red:
            print(f"Avg red filter           : {avg_red:.3f} s")
...
def model_grounding_SAM(samples, mode = 'base', save_imgs_bool=False, store_masks_bool=False, testing_samples=1, filter_bboxes=(True, 0.2, 3.0), filter_masks_shapes=(True, 0.85), filter_masks_sizes=(True, 0.2, None), filter_red=(True, 0.4), filter_holes_islands = False, filter_overlap_masks = False, filter_by_darkness = False):
   ...
   # device = 'cuda' if torch.cuda.is_available() else 'cpu'
    ...
    time_dino_load = time() - _t0
    print(f"Grounding DINO loading time (one-off): {time_dino_load:.2f} seconds")
    print(f"Grounding DINO parameter dtypes: { {p.dtype for p in object_detector.model.parameters()} }")
    labels = ["raspberry."]
    ...
    time_sam_load = time() - _t0
    print(f"SAM model loading time (one-off): {time_sam_load:.2f} seconds")
    ...
    time_depth_load = time() - _t0
    print(f"Depth pipeline loading time (one-off): {time_depth_load:.2f} seconds")
    print(f"Depth-Anything parameter dtypes: { {p.dtype for p in pipe.model.parameters()} }")
  ...
  #  model.eval()
    ...
    for loop_idx, sample in enumerate(samples):
      ...
      #  raw_img = np.array(sample_img)[:, :, ::-1]  # RGB to BGR
       ...
       # plt.show()
        ...
        if loop_idx == 0:
            print(f"SAM parameter dtypes: { {p.dtype for p in mask_predictor.model.parameters()} }")
        if loop_idx > 0:
    ...
    if times_image_encoding:
        ...
        print("\n--- Timing Summary (excluding first sample) ---")
        print(f"DINO load time (one-off) : {time_dino_load:.3f} s")
        print(f"SAM load time (one-off)  : {time_sam_load:.3f} s")
        print(f"Depth load time (one-off): {time_depth_load:.3f} s")
        print(f"Avg depth estimation     : {avg_depth:.3f} s")
...
### FILTERS
# Of all the filters, in the end only the red masks and bbox sizes filter is used. The other ones can be ignored
...
# Depth based overlapping masks filter
...
# Helper for overlapping masks filter
...
# Storing masks for creating dataset later on
...
def main():
    ...
    if MODE == "SAM_extended":
        ...
        model_SAM_extended(full_data, mode = 'base', save_imgs_bool = False, store_masks_bool = False, testing_samples = 'all', filter_bboxes = (True, None, 3.0), filter_masks_shapes = (False, 0.85), filter_masks_sizes = (False, 0.2, None), filter_red = (True, 0.3), filter_holes_islands = True, filter_overlap_masks = True)
    
    elif MODE == "grounding_SAM":
        model_grounding_SAM(full_data, mode = 'base', save_imgs_bool = False, store_masks_bool = False, testing_samples = 'all', filter_bboxes = (True, None, 3.0), filter_masks_shapes = (False, 0.85), filter_masks_sizes = (False, 0.2, None), filter_red = (True, 0.3), filter_holes_islands = True, filter_overlap_masks = True, filter_by_darkness = False)

    elif MODE == "SAM3":
        model_SAM3(full_data, save_imgs_bool = False, store_masks_bool = True, testing_samples = 'all', filter_masks_shapes = (False, 0.95), filter_bboxes = (True, None, 3.0), filter_masks_sizes = (False, 0.2, None), prompt = "raspberry", filter_red = (True, 0.3), filter_holes_islands = True, filter_overlap_masks = True)
